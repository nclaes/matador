...
{chr(10).join(f"            assign hcb_valid[{i}] = fifo_m_tvalid & (state == S_RECV) & (beat_cnt == {i});" for i in range(NB))}
    ...
    def _gen_verilator_makefile(self) -> str:
        # Verilator exits non-zero on ANY unsuppressed warning by default,
        # not just genuine errors -- these three are expected, harmless
        # noise from this generator's own output, the same category
        # --Wno-WIDTH/--Wno-BLKSEQ above already represent: UNUSEDPARAM
        # (module parameters threaded through uniformly for every
        # instance, not every one read by every submodule body),
        # UNUSEDSIGNAL (fixed-width intermediate signals/bus bits not all
        # read at every pipeline stage), SYNCASYNCNET (axis_fifo.v's reset
        # is intentionally async, hw_tm_accelerator.v's own FSM reset
        # check is intentionally synchronous -- a deliberate mix, not a
        # bug). --Wno-DECLFILENAME is permanent, not a suppression of a
        # fixable issue: hcb_blocks.v legitimately contains many
        # differently-named HCB_i modules, so "filename == module name"
        # can never hold for it.
