...
class TMAccelerator:
    ...
    def _gen_tm_top(self) -> str:
        ...
        return (
            ...
            f"//\n"
            f"//   S_COMPUTE {NFS} × {NCS} = {NFS*NCS} cycles.\n"
            f"//             feat_cnt (outer, 0→{NFS-1}) and clause_slice_cnt (inner, 0→{NCS-1})\n"
            f"//             index into the tile ROM. Each cycle, {CS} clause_eval instances\n"
            f"//             evaluate {CS} clauses against the current {FS}-feature window.\n"
            f"//             Results are AND-accumulated into clause_pass[] and OR-accumulated\n"
            f"//             into clause_has_actions[]. Empty tiles vacuously pass:\n"
            ...
            f"//   Width    = {TW} bits  ({CS} clauses × 2 × {FS} literals)\n"
            f"//   bit[k×{2*FS}+l]      Include for positive literal l, clause k-in-tile\n"
            f"//   bit[k×{2*FS}+{FS}+l] Include for negated  literal l, clause k-in-tile\n"
            f"//   Matches partial_lits = {{~feat_pos, feat_pos}} so clause_eval needs no\n"
            ...
            f"                end\n"
            f"\n"
            f"                // ─── Tile-by-tile partial clause evaluation ───────────────\n"
            f"                // For each tile, CLAUSE_SLICE clause_eval instances compute\n"
            f"                // tile_pass[] and tile_has_actions[] combinatorially this cycle.\n"
            f"                S_COMPUTE: begin\n"
            ...
            f"                end\n"
            f"\n"
            f"                // ─── Sequential score accumulation ───────────────────────\n"
            ...
            f"                end\n"
            f"\n"
            f"                // ─── Output result via AXI-Stream, then clear scores ──────\n"
    ...
    def _gen_verilator_makefile(self) -> str:
        # --build tells Verilator to compile and link in one shot, so there is
        # no intermediate Vtm_accelerator.mk with baked-in absolute paths.
        # This makes the directory relocatable: copy it anywhere, run make.
        return textwrap.dedent("""\
        # Auto-generated by matador rtl generate — Verilator build for tb_system
        #
        # Portable: uses $(SCRIPT_DIR) computed from the Makefile's own location,
        # so this directory can be copied or moved without editing any paths.
        VERILATOR  ?= verilator
        ...
        # Do NOT add a separate $(MAKE) -C obj_dir step — --build handles it.
        # --CFLAGS "-std=c++17": Apple Clang defaults to C++98 without an explicit
        # standard flag, causing Verilator's own headers to fail to compile.
        VFLAGS := \\
