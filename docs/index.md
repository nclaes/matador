...
registered dataset, just described by you — see [Usage](Usage.md).
...
| Document | Contents |
|---|---|
| [Setup](Setup.md) | Docker setup, building the container, first run |
| [Usage](Usage.md) | Full ingest → booleanize → train → generate → simulate workflow with YAML examples |
| [Generated Outputs](GeneratedOutputs.md) | A map of `/work` — what every command writes, what each file is for, what's safe to delete |
| [Developer Guide](Developer.md) | Extending Matador — adding a new dataset, booleanisation technique, or accelerator backend |
| [TMIR Spec](tmir_spec.md) | The Tsetlin Machine Intermediate Representation every backend consumes |
...
`data/Raw_Data_Bank.yaml` (a plain YAML file: where to fetch
...
new Boolean encoding technique or a new accelerator backend. The [Developer Guide](Developer.md)
has a worked example for each:

- [Adding a new dataset](Developer.md#adding-a-new-dataset)
- [Adding a new booleanisation technique](Developer.md#adding-a-new-booleanisation-technique)
- [Adding a new accelerator backend](Developer.md#adding-a-new-accelerator-backend)
...
See [Adding a new dataset](Developer.md#adding-a-new-dataset) to add another one.
...
See [Adding a new accelerator backend](Developer.md#adding-a-new-accelerator-backend) to add another one.
