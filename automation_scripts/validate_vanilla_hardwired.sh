#!/usr/bin/env bash
#
# validate_vanilla_hardwired.sh -- backend-specific validation for
# vanilla_hardwired: trains one model, generates fresh RTL parameterized
# to exactly that model, then compiles/runs it through BOTH real
# simulators and the software emulator.
#
...
#
# Usage:
#   ./automation_scripts/validate_vanilla_hardwired.sh [WORK_DIR]
#   WORK_DIR=/work ./automation_scripts/validate_vanilla_hardwired.sh
#
...
#   T_THRESHOLD     voting threshold (default: 200) -- vanilla_hardwired
#                   has no known hard ceiling on this the way
#                   vanilla_gp_tiled's vendored core does (SCORE_WIDTH=6,
#                   T<=31), so the standard template default is safe here.
#
# Output: $WORK_DIR/automation_report/vanilla_hardwired/ (report.md, report.json, logs/)
#
# Note on `matador emulate --verify`: deliberately NOT used here. Its
# cross-check (matador.verification.compare.compare_emulator_to_reference)
# hardcodes the vanilla_tiled emulator class specifically and is a known,
# documented, not-yet-fixed limitation for every OTHER backend, including
# this one (see docs/Developer.md's "Known limitation" note under The
# three-layer contract). Plain `matador emulate` (no --verify) still gives
# a real, meaningful check: each vector's emulator prediction is compared
# against that vector's OWN embedded expected_class, and the CLI exits
# non-zero on any mismatch -- exactly what run_step() below checks.
...
if run_step "$BACKEND" "generate" "$LOG_DIR/generate.log" \
    ...
    run_step "$BACKEND" "emulate" "$LOG_DIR/emulate.log" \
        matador emulate --backend "$BACKEND" --config "$accel_cfg"
else
