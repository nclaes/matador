...
Both `generate` and `testbench` also (re)write a self-documenting
`RTL/README.md` alongside the sources — written for whoever receives just
that directory (an RTL engineer, most likely), not someone running
`coal_tm` themselves — see
[What `generate`/`testbench` actually write](#what-generatetestbench-actually-write)
below.
...
# Same simulation, standalone -- no coal_tm/Python needed, just
# iverilog/verilator + gtkwave (see /work/mnist_model/RTL/README.md, which
# `testbench` just (re)wrote, for the full waveform-viewing walkthrough):
cd /work/mnist_model/RTL/tb && ./run_iverilog.sh && gtkwave iverilog.vcd
```
...
## What `generate`/`testbench` actually write

```
<output_dir>/RTL/
  TM_Hard_Coded_Clause_Blocks.sv   generated, sensitive to trained values
  hard_coded_weight.sv             generated, sensitive to trained values
  HCB_top.sv                       generated, sensitive to model shape only
  TM_top.sv                        generated, sensitive to model shape only
  axis_wrapper.sv                  generated, sensitive to model shape only
  AXI_Interface.sv                 static IP, copied verbatim every run
  new_adder.sv                     static IP, copied verbatim every run
  TM_argmax.sv                     static IP, copied verbatim every run
  README.md                        self-documenting -- see below
  tb/                              written by `testbench`, not `generate`
    testbench.sv                   self-checking, sized from real config
    run_iverilog.sh                standalone, no coal_tm/Python required
    run_verilator.sh               standalone, no coal_tm/Python required
  sim/                             written by `testbench`, not `generate`
    stimulus.mem
    expected.mem
```

`RTL/README.md` is regenerated by *both* `generate` and `testbench`, so
it always reflects what's actually on disk rather than the two commands
independently guessing at each other's output (running `generate` again
after `testbench` doesn't wipe out the testbench-specific parts of the
README as long as `tb/`/`sim/` are still there). It documents, for this
exact bundle: the model parameters that shaped it; which of the 5
generated files are sensitive to the model's *trained values* versus its
*shape* only (with `grep`/`wc` commands to verify this yourself, no
`coal_tm`/Python needed); and, once `tb/run_iverilog.sh`/
`tb/run_verilator.sh` exist, how to run them and open the resulting
`.vcd` waveforms in GTKWave (which signals to add, and the module path
into the DUT for the clause/adder/argmax internals).

Run the [zero-setup first run](#zero-setup-first-run-using-the-repos-own-bundled-model)
above to generate a real one and read it yourself — it's plain Markdown,
`/work/mnist_model/RTL/README.md` on your host once you've run `generate`.
